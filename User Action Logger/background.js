// background.js — User Activity Logger v1
...
const STORAGE_KEY_LOGS     = 'session_logs';
const STORAGE_KEY_EXPORTED = 'exported_logs'; // all-time cumulative log

async function initIds() {
  const [local, session] = await Promise.all([
    chrome.storage.local.get(['user_id', 'device_id', STORAGE_KEY_LOGS]),
    chrome.storage.session.get(['ual_enabled']),
  ]);
  ...
  if (typeof session.ual_enabled === 'boolean') {
    loggingEnabled = session.ual_enabled;
  }
  ...
  _queue.length = 0;
  console.log('[User Activity Logger] Ready. user_id:', USER_ID, '| rehydrated logs:', sessionLogs.length);
}
...
// ── Internal URL filter ───────────────────────────────────────────────────────
// Covers Chrome, Edge, Firefox, Opera, Brave and other Chromium forks.
const INTERNAL_PREFIXES = [
  'chrome://', 'chrome-extension://',
  'edge://', 'edge-extension://',
  'about:', 'data:', 'javascript:',
  'brave://', 'opera://', 'vivaldi://', 'yandex://',
];
...
function getBrowser() {
  ...
  if (ua.includes('opr/') || ua.includes('opt/')) return 'opera';
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'set_enabled') {
    ...
    chrome.storage.session.set({ ual_enabled: msg.value });
...
function triggerDownload(sendResponse) {
  ...
  chrome.storage.local.get([STORAGE_KEY_EXPORTED], stored => {
    ...
    const dataUrl = 'data:application/x-json;base64,' + b64;
